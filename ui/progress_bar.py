...
class NX_Progress_Bar:
    ...
    def __init__(self, x, y, width, height, progress, color, label="Building Lightmaps"):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
    ...
    def draw(self):
        ...
        if progress_width < 0.01:
            label_text = f"{self.label}: Starting"
        elif progress_width > 99.9:
            label_text = f"{self.label}: Done"
        else:
            pct = str(round(progress_width)) + "%"
            eta_part = f" (Estimated: {self.eta})" if self.eta else ""
            label_text = f"{self.label}: {pct}{eta_part}"
        ...
        # Setting up and drawing the text to the right of the bar
        ...
        blf.draw(self.font_id, label_text)
