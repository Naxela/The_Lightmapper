...
class ArmoryAddonPreferences(AddonPreferences):
    ...
    def gimp_path_update(self, context):
            if self.skip_update or self.gimp_path == '':
                return
            self.skip_update = True
            self.gimp_path = bpy.path.reduce_dirs([bpy.path.abspath(self.gimp_path)])[0]
    ...
    gimp_path: StringProperty(name="GIMP Path", description="GIMP binary path", subtype="FILE_PATH", update=gimp_path_update, default="")
